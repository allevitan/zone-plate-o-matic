...
__all__ = ['FFT_DI', 'FFT_DI_tiled', 'create_G',
           'create_G_t',
           'get_num_tiles',
...
def create_G_t(U_0, z, wavelength, step, A, t, offset=[0,0], t_0=None):
    ...
    The output is the array G, expressed in real space, as defined
    in Shen and Wang (2006), doi:10.1364/AO.45.001102, and then
    extended to time-dependent propagation by me (Abe Levitan)

    The assumption is that the input wavefield is separable into the
    product of a spatially-dependent wavefield (U_0) and a time-dependent
    envelope function A(t). A(t) is assumed to vary much more slowly than the 
    central frequency defined by the wavelength.

    A should be provided as a function which can accept a 2D tensor of time
    delays.

    An optional time offset, t_0, can be set for convenience. The default
    time offset is the time taken for light to travel the distance z.
    ...
    offset : array
        ...
        of the input array to corner of the output array.
    t_0 : float
        Optional, a time offset. Default is z / (speed of light).
...
def FFT_DI_t(U_0, z, wavelength, step, A, t, offset=[0,0], t_0=None,
    ...
    This function performs light propagation under open boundary conditions
    using FFT-DI (Shen and Wang (2006), doi:10.1364/AO.45.001102), and then
    extended to time-dependent propagation by me (Abe Levitan)
...
def FFT_DI_tiled_t(U_0, z, wavelength, step, A, t,
    ...
    """Uses a time-dependent variation of FFT-DI to propagate a light field, breaking the problem into tiles.

    Because FFT-DI uses open boundary conditions, it's easy to break a large
    problem into many subproblems by tiling both the input and output planes.
    This remains true in the time-dependent formulation.
